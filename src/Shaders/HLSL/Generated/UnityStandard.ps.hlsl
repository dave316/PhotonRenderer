...
void surf(Input i, inout SurfaceOutputStandardSpecular o)
{
	...
	float2 var9 = float2(_DetailAlbedoMap_ST.r, _DetailAlbedoMap_ST.g);
	float2 var10 = float2(_DetailAlbedoMap_ST.b, _DetailAlbedoMap_ST.a);
	float4 var11 = var4 * _Color;
	float var12 = lerp(1.0, var6.g, _OcclusionStrength);
	float4 var13 = _SPECGLOSSMAP ? var7 : _SpecColor;
	float var14 = _SPECGLOSSMAP ? var7.a : _Glossiness;
	float2 var15 = i.uv_texcoord * var9 + var10;
	o.Occlusion = var12;
	o.Specular = (float3)var13;
	o.Smoothness = var14;
	float4 var19 = tex2D(_DetailAlbedoMap, var15);
	float4 var20 = UnpackNormal4(_DetailNormalMap, var15);
	float4 var21 = var19 * var1;
	float3 var22 = ScaleNormal((float3)var20, _DetailNormalMapScale);
	float4 var23 = lerp(float4(1.0, 1.0, 1.0, 1.0), var21, (float4)var8.a);
	float3 var24 = BlendNormals(var22, (float3)var5);
	float4 var25 = var23 * var11;
	float3 var26 = lerp((float3)var5, var24, (float3)var8.a);
	o.Albedo = (float3)var25;
	o.Normal = var26;
}
...
float3 f_specular = radianceAnisotropy(n, v, surface.roughness, surface.anisotropyStrength, surface.anisotropicBitangent);
    ...
    if (length(n) > 0.0) // TODO: this is a workaround to render meshes unlit when the normal is zero
        fragColor = float4(tex2D(_MainTex, input.texCoord0).rgb, surface.alpha);
    else
