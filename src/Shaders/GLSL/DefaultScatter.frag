...
void main()
{
	...
	fragColor = vec4(0.0, 0.0, 0.0, 1.0); // TODO: Get material ID in alpha channels!
	...
	vec3 diffuseTransmissionIBL = radianceLambert(-n) * surface.translucency;
	...
	vec3 f_dielectricFresnelIBL = fresnelGGX(n, v, surface.roughness, surface.F0, surface.specularWeight);
	fragColor += vec4(mix(f_diffuse, f_specular_dielectric, f_dielectricFresnelIBL), 0.0) * albedoSheenScaling;
...
#endif
		vec3 dielectricBRDF = mix(f_diffuse, vec3(0), dielectricFresnel);
		directColor = dielectricBRDF * albedoSheenScaling;
	}

	fragColor += vec4(directColor, 0.0);
}