...
//	return transmittance * transmittedLight;
	...
	vec3 transmittance = pow(attenuationColor, vec3(transmittedDistance / attenuationDistance));
	return transmittance * transmittedLight;
}
...
vec3 getSubsurfaceScattering(vec3 position, mat4 projection, float attenuationDistance, vec3 diffuseColor, vec3 multiScatterColor)
{
	...
	vec2 uv = gl_FragCoord.xy / vec2(textureSize(scatterDepthTexture, 0));
	...
	vec4 upos = inverseProjection * clipSpacePosition;
	vec3 fragViewPosition = upos.xyz / upos.w;
	upos = inverseProjection * vec4(clipUV.x + texelSize.x, clipUV.y, centerDepth, 1.0);
	...
	for (int i = 0; i < SCATTER_SAMPLE_COUNT; i++)
	{
		...
		if (centerSample.w == textureSample.w)
		{
			...
			vec4 sampleUPos = inverseProjection * vec4(sampleClipUV.x, sampleClipUV.y, sampleDepth, 1.0);
...
#ifdef TRANSLUCENCY
	vec3 ssColor = getSubsurfaceScattering(wPosition, camera.projection, surface.attenuation.a, surface.translucencyColor, surface.multiScatter);
	color += ssColor;
#endif
