...
#define _Detail_Albedo_ST material._Detail_Albedo_ST
...
#define _Detail_Albedo material._Detail_Albedo
...
void surf( Input i , inout SurfaceOutputStandard o )
{
	...
	vec3 tex2DNode3 = UnpackScaleNormal( tex2D( _Normals, uv_Albedo ) ,_NormalPower );
	o.Normal = BlendNormals( UnpackScaleNormal( tex2D( _Normals_Detail, uv_Normals_Detail ) ,_NormalDetailPower ) , tex2DNode3 );
	vec2 uv_Detail_Albedo = i.uv_texcoord * _Detail_Albedo_ST.xy + _Detail_Albedo_ST.zw;
	...
	vec4 tex2DNode2 = tex2D( _Metallic, uv_Albedo );
	...
	vec4 temp_cast_2 = (tex2D( _Occlusion, uv_Occlusion ).g).xxxx;
	vec4 mixResult56 = mix( vec4(1,1,1,0) , temp_cast_2 , _OcclusionPower);
	o.Occlusion = mixResult56.r;
	o.Alpha = 1;
}
