...
#define _NormalMap_ST material._NormalMap_ST
#define _Color material._Color
#define _Albedo_ST material._Albedo_ST
#define _Smoothness_ST material._Smoothness_ST
#define _Occlusion_ST material._Occlusion_ST
#define _NormalScale material._NormalScale
#define _Metallic material._Metallic
#define _Occlusion_Power material._Occlusion_Power
#define _Opacity material._Opacity
#define _ChromaticAberration material._ChromaticAberration
#define _IndexofRefraction material._IndexofRefraction
#define _NormalMap material._NormalMap
...
#define _GrabTexture grabTexture
 ...
 vec4 Refraction( Input i, SurfaceOutputStandard o, float indexOfRefraction, float chomaticAberration ) {
	vec3 worldNormal = o.Normal;
	vec4 screenPos = i.screenPos;
	...
	#endif
	vec2 projScreenPos = ( screenPos / screenPos.w ).xy;
	vec3 worldViewDir = normalize( UnityWorldSpaceViewDir( i.worldPos ) );
	vec3 refractionOffset = ( ( ( ( indexOfRefraction - 1.0 ) * mul( UNITY_MATRIX_V, vec4( worldNormal, 0.0 ) ) ) * ( 1.0 / ( screenPos.z + 1.0 ) ) ) * ( 1.0 - dot( worldNormal, worldViewDir ) ) );
	vec2 cameraRefraction = vec2( refractionOffset.x, -( refractionOffset.y * _ProjectionParams.x ) );
	vec4 redAlpha = tex2D( _GrabTexture, ( projScreenPos + cameraRefraction ) );
	float green = tex2D( _GrabTexture, ( projScreenPos + ( cameraRefraction * ( 1.0 - chomaticAberration ) ) ) ).g;
	float blue = tex2D( _GrabTexture, ( projScreenPos + ( cameraRefraction * ( 1.0 + chomaticAberration ) ) ) ).b;
	return vec4( redAlpha.r, green, blue, redAlpha.a );
}

void RefractionF( Input i, SurfaceOutputStandard o, inout vec4 color )
{
	#ifdef UNITY_PASS_FORWARDBASE
	color.rgb = color.rgb + vec3(Refraction( i, o, _IndexofRefraction, _ChromaticAberration )) * ( 1 - color.a );
	color.a = 1;
	#endif
}
...
{
	o.Normal = vec3(0,0,1);
	vec2 uv_NormalMap = i.uv_texcoord * _NormalMap_ST.xy + _NormalMap_ST.zw;
	...
	vec4 tex2DNode56 = tex2D( _Albedo, uv_Albedo );
	o.Albedo = ( _Color * tex2DNode56 ).rgb;
	o.Metallic = _Metallic;
	vec2 uv_Smoothness = i.uv_texcoord * _Smoothness_ST.xy + _Smoothness_ST.zw;
	o.Smoothness = tex2D( _Smoothness, uv_Smoothness ).a;
	vec2 uv_Occlusion = i.uv_texcoord * _Occlusion_ST.xy + _Occlusion_ST.zw;
	float mixResult64 = mix( 1 , tex2D( _Occlusion, uv_Occlusion ).g , _Occlusion_Power);
	o.Occlusion = mixResult64;
	o.Alpha = ( tex2DNode56.a * _Opacity );
	o.Normal = o.Normal + 0.00001 * vec3(i.screenPos) * i.worldPos;
}
...
{
	RefractionF(unityInput, unitySurface, color);
	return color;
}
