...
namespace Math
{
	...
	Frustrum::Frustrum(FPSCamera& camera)
	{
		...
		nearP = Plane(p + zNear * f, f);
		farP = Plane(p + frontMultFar, -f);
	...
	glm::mat4 getLightSpaceMatrix(UserCamera& camera, glm::vec3& lightDir, float zNear, float zFar)
	{
		...
		if (pr::GraphicsContext::getInstance().getCurrentAPI() == pr::GraphicsAPI::Direct3D11)
			return glm::transpose(lightProj * lightView);
		else
	...
	std::vector<glm::mat4> creatCMViews(glm::vec3 position, float zNear, float zFar)
	{
