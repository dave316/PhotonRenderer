...
Animation::Animation(const std::string& name, Interpolation interp, float ticksPerSecond, float startTime, float endTime, float duration, unsigned int nodeIndex) :
	name(name),
	interpMode(interp),
	currentTime(0.0f),
...
void Animation::setTimes(std::vector<float>& times)
{
	this->times = times;
}

void Animation::setPositions(std::vector<glm::vec3>& positions)
{
	this->positions = positions;
}

void Animation::setRotations(std::vector<glm::quat>& rotations)
{
	this->rotations = rotations;
}

void Animation::setScales(std::vector<glm::vec3>& scales)
{
	this->scales = scales;
}

unsigned int Animation::findTime(float currentTime)
{
