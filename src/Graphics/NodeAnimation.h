#ifndef INCLUDED_NODEANIMATION
#define INCLUDED_NODEANIMATION
...
private:
	...
	NodeAnimation(const NodeAnimation&) = delete;
	NodeAnimation& operator=(const NodeAnimation&) = delete;
public:
	NodeAnimation();
	NodeAnimation(const std::string& name, Interpolation interp, float ticksPerSecond, float startTime, float endTime, float duration, unsigned int nodeIndex);
	~NodeAnimation();
	...
	typedef std::shared_ptr<NodeAnimation> Ptr;
	static Ptr create()
	{
		return std::make_shared<NodeAnimation>();
	}
};

#endif // INCLUDED_NODEANIMATION